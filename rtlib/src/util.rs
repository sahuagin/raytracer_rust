...
pub fn random_scene(rng: &mut impl rand::Rng) -> HitList {
    ...
    hl.add( 
        ...
        Lambertian::new(
            &color_to_texture!(&vect!(0.5, 0.5, 0.5)))))));
    for a in -11..11 {
        for b in -11..11 {
            ...
            if (center - vect!(4.0, 0.2, 0.0)).length() > 0.9 {
                if choose_mat < 0.8 { // diffuse
                    hl.add( Hitters::Sphere(Sphere::new( &center, 0.2, 
                        MaterialType::Lambertian(Lambertian::new(
                        &color_to_texture!(&vect!(rng.gen::<f64>()*rng.gen::<f64>(),
    ...
    hl.add(Hitters::Sphere(Sphere::new(
        &vect!(
         ...
         1.0,
         wrap_material!(Dielectric, &vect!(1.0, 1.0, 1.0), 1.5))));
    hl.add(Hitters::Sphere(Sphere::new( &vect!(-4.0, 1.0, 0.0), 1.0,
                        wrap_material!(Lambertian, &color_to_texture!(&vect!(0.4, 0.2, 0.1))))));
    hl.add(Hitters::Sphere(Sphere::new( &vect!(4.0, 1.0, 0.0), 1.0,
                        wrap_material!(
                            Metal, 
                                color_to_texture!(&vect!(0.7, 0.6, 0.5)), 0.0))));
